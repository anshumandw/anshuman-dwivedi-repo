...
public class ContestService implements IContestService {
    ...
    private List<Question> pickQuestionsList(final List<Question> questions,final Integer numQuestion){
        ...
        List<Question> questionsList = questions.stream().collect(Collectors.toList());
        ...
        for(int i=0; i<numQuestion; i++) {
            ...
            quesList.add(questionsList.get(randomIndex));
    ...
    public List<Contest> getAllContestLevelWise(Level level) {
        List<Contest> list = contestRepository.findAllContestLevelWise(level);
        if(level == null) {
            list = contestRepository.findAll();
        }
        return list;
    }
